...
`balanced` 这组更适合 OpenClaw / coding agent 的选型说明见 [docs/openrouter-balanced-profile.md](/Users/jay/clawgo/docs/openrouter-balanced-profile.md)。
`value` 这组更偏性价比的选型说明见 [docs/openrouter-value-profile.md](/Users/jay/clawgo/docs/openrouter-value-profile.md)。
...
```bash
openclaw models set clawgo/balanced
```
...
Options:
  ...
  --debug-transcript 打印可读的 LLM 对话 transcript
  --version, -v  显示版本
  --help, -h     显示帮助
```
...
完整示例见 [config.example.yaml](/Users/jay/clawgo/config.example.yaml)。
`balanced` 的选型依据见 [docs/openrouter-balanced-profile.md](/Users/jay/clawgo/docs/openrouter-balanced-profile.md)。
`value` 的选型依据见 [docs/openrouter-value-profile.md](/Users/jay/clawgo/docs/openrouter-value-profile.md)。
...
配置后可通过 `profile: balanced` 设为默认路由配置，或在请求里使用 `model: "balanced"` / `model: "clawgo/balanced"` 显式选择该层级映射。`balanced` 这组示例更适合 OpenClaw 这类带工具调用和代码上下文的 agent。
